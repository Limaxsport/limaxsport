...
const slowConnectionDetector = {
    timerId: null,
    element: null,
    start: function (elementId, timeout = 7000) { // Чекаємо 7 секунд перед показом
        this.stop(); // Зупиняємо попередній таймер, якщо він був
        this.element = document.getElementById(elementId);

        this.timerId = setTimeout(() => {
            if (this.element) {
                this.element.innerHTML = '⏳ Схоже, у вас повільний Інтернет-зв\'язок. Зачекайте, будь ласка...';
...
let currentUserWorkoutPage = 0; // Лічильник завантажених "сторінок"
...
let areTimerSoundsEnabled = localStorage.getItem('timerSoundsEnabled') !== 'false';
...
function setTokens(accessToken, refreshToken, expiresInSeconds) {
    ...
    if (refreshToken) { // Рефреш токен може не змінюватися при кожному оновленні
    ...
    if (expiresInSeconds) {
        ...
        console.warn(`expires_in не надано, встановлено дефолтний час життя access token до: ${new Date(accessTokenExpiresAt).toLocaleString()}`);
    }
...
function clearTokens() {
    ...
    if (proactiveRefreshTimerId) { // Очищуємо таймер проактивного оновлення
...
function scheduleProactiveTokenRefresh() {
    ...
    if (!expiresAt || !getRefreshToken()) { // Немає даних для оновлення або рефреш токена
    ...
    if (refreshInMs < 0) { // Якщо час вже минув або дуже близько
        refreshInMs = 5000; // Спробувати оновити через 5 секунд, якщо вже "пора"
        console.warn(`Час для планового оновлення токена вже близько або минув. Спроба через ${refreshInMs / 1000}с.`);
    }
    ...
    proactiveRefreshTimerId = setTimeout(handleProactiveTokenRefresh, refreshInMs);
}
...
async function handleProactiveTokenRefresh() {
    ...
    try {
        ...
        console.error("Проактивне оновлення токена остаточно не вдалося після всіх спроб:", error);
        ...
        if (error && error.message && error.message.toLowerCase().includes('invalid_grant')) {
            console.warn("Невалідний refresh token при проактивному оновленні. Вихід з системи.");
            clearTokens();
            updateVisibility();
            alert("Сесія закінчилася через недійсний токен оновлення. Будь ласка, увійдіть знову.");
        }
...
async function performTokenRefreshWithRetries() {
    if (isRefreshing) { // Додаткова перевірка
    ...
    try {
        const newTokens = await retryOperation(async () => {
            const refreshTokenValue = getRefreshToken();
            if (!refreshTokenValue) {
                console.warn("performTokenRefreshWithRetries: Немає refresh token для оновлення.");
            ...
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({ detail: `Помилка сервера при оновленні: ${response.status}` }));
                // Якщо помилка "invalid_grant", не потрібно робити ретраї
                if (response.status === 400 || response.status === 401 || (errorData.detail && errorData.detail.toLowerCase().includes('invalid_grant'))) {
                    console.error("performTokenRefreshWithRetries: Невалідний refresh token або інша критична помилка:", errorData.detail);
                    throw { ...new Error(errorData.detail || "Invalid refresh token"), noRetry: true };
                }
                console.warn(`performTokenRefreshWithRetries: Спроба оновлення не вдалася (статус ${response.status}), спроба повтору...`);
                throw new Error(errorData.detail || `Refresh failed with status ${response.status}`);
            }
            return response.json();
        }, RETRY_ATTEMPTS, RETRY_INITIAL_DELAY_MS, RETRY_BACKOFF_FACTOR);

        //console.log("performTokenRefreshWithRetries: Отримано нові токени:", newTokens);
        setTokens(newTokens.access_token, newTokens.refresh_token, newTokens.expires_in); // expires_in з бекенду
        ...
        console.error("performTokenRefreshWithRetries: Критична помилка після всіх спроб оновлення токена:", error);
        clearTokens();
        updateVisibility();
 ...
 */
async function retryOperation(operation, retries, delayMs, backoffFactor, attempt = 1) {
    try {
        return await operation();
    } catch (error) {
        if (error.noRetry || attempt >= retries) { // Якщо помилка не для ретраю або спроби закінчились
            console.error(`retryOperation: Остання спроба (${attempt}/${retries}) не вдалася або помилка не для ретраю.`, error);
            throw error;
        }
        console.warn(`retryOperation: Спроба ${attempt}/${retries} не вдалася. Повтор через ${delayMs}ms. Помилка:`, error.message);
        await new Promise(resolve => setTimeout(resolve, delayMs));
        return retryOperation(operation, retries, delayMs * backoffFactor, backoffFactor, attempt + 1);
    }
}
...
// === НОВІ ФУНКЦІЇ ДЛЯ РОБОТИ З WEB AUDIO API ===
...
/**
 * Оновлено: Ініціалізує звуки один раз і встановлює прапорці.
 */
async function initAudio() {
    ...
    try {
        [tickBuffer, finishBuffer, finish2Buffer] = await Promise.all([
            loadAudioData('https://limaxsport.top/static/sounds/timer-tick.mp3?v=1.1'),
            loadAudioData('https://limaxsport.top/static/sounds/timer-finish.mp3?v=1.2'),
            loadAudioData('https://limaxsport.top/static/sounds/timer-finish2.mp3?v=1.1')
        ]);
        ...
        alert("Не вдалося завантажити звуки для таймера. Спробуйте оновити сторінку.");
    } finally {
...
/**
 * ФІНАЛЬНА ВЕРСІЯ: Надійно відтворює звук і викликає колбек через setTimeout.
...
/**
 * ОНОВЛЕНО: Наполегливо "будить" AudioContext. Безпечно викликати багато разів.
 */
...
function getWeekdaysText(values) {
    ...
    return values.sort((a, b) => (dayOrder[a] || 8) - (dayOrder[b] || 8)).join(', ');
}

function getHealthProblemsText(values) { // values - це масив
...
const productNameMap = {
    "milk": "Молоко", "cottage_cheese": "Творог", "yogurt": "Йогурт", "sour_cream": "Сметана", "cheese": "Сир", "eggs": "Яйця",
    "chicken": "Курка", "turkey": "Індичка", "pork": "Свинина", "beef": "Яловичина", "shank": "Шинка", "offal": "Субпродукти",
    "salted_fish": "Солона риба", "cooked_fish": "Риба приготовлена", "shrimps": "Креветки", "squid": "Кальмари", "caviar": "Ікра",
    "cod_liver": "Печінка тріски", "canned_fish": "Консервована риба", "oatmeal": "Вівсянка", "buckwheat": "Гречка",
    "rice": "Рис", "bulgur": "Булгур", "pasta": "Макарони", "spaghetti": "Спагетті", "corn_grits": "Кукурудзяна крупа", "quinoa": "Кіноа",
    "couscous": "Кускус", "semolina": "Манна крупа", "pearl_barley": "Перлівка", "millet": "Пшоно", "barley_groats": "Ячна крупа", "flakes": "Пластівці", "potato": "Картопля",
    "sweet_potato": "Батат", "bread": "Хліб", "pita": "Лаваш", "tortilla": "Тортілья", "breadsticks": "Хлібці", "nuts": "Горіхи",
    "peanut_butter": "Арахісова паста", "peas": "Горох", "lentils": "Сочевиця", "beans": "Квасоля", "butter": "Масло вершкове",
    "olive": "Оливки", "mushrooms": "Гриби", "beet": "Буряк", "onion": "Цибуля", "tomatoes": "Помідори (томатна паста)", "canned_vegetables": "Консервовані овочі",
    "zucchini": "Кабачки", "eggplants": "Баклажани", "pumpkin": "Гарбуз", "avocado": "Авокадо", "banana": "Банани", "apples": "Яблука",
    "pears": "Груші", "orange": "Апельсини", "lemon": "Лимони", "kiwi": "Ківі", "strawberry": "Полуниця", "dried_fruits": "Сухофрукти",
    "jam": "Варення/джем", "marshmallow": "Зефір", "lukum": "Лукум", "protein": "Протеїн (спортпіт)"
};

function getProductsText(values) { // values - це масив англійських ключів
    if (!values || !Array.isArray(values) || values.length === 0) {
    ...
    return values.map(value => {
        ...
        return productNameMap[value] || value.charAt(0).toUpperCase() + value.slice(1).replace(/_/g, ' ');
    }).join(', ');
}
...
const USER_REPS_OPTIONS = '<option value="">--</option>' + Array.from({ length: 50 }, (_, i) => `<option value="${i + 1}">${i + 1}</option>`).join('');
const USER_WEIGHT_OPTIONS = '<option value="">--</option>' + Array.from({ length: 300 }, (_, i) => `<option value="${i + 1}">${i + 1} кг</option>`).join('');
const USER_TIME_OPTIONS = '<option value="">--</option>' + Array.from({ length: 60 }, (_, i) => { // від 5 до 300 секунд
    const seconds = (i + 1) * 5;
...
function displayStatus(elementId, message, isError = false, clearAfterMs = 0) {
    ...
    if (statusDiv) {
        ...
        console[isError ? 'error' : 'warn'](`Елемент статусу з ID "${elementId}" не знайдено. Повідомлення: ${message}`);
    }
}
...
async function fetchWithAuth(url, options = {}) {
    ...
    try {
        ...
        if (response.status === 401 && getRefreshToken()) {
            if (!isRefreshing) {
                ...
                try {
                    ...
                    console.error("fetchWithAuth: Помилка оновлення токена після 401.", refreshError);
                ...
                fetchOptions.headers['Authorization'] = `Bearer ${newAccessTokenFromQueue}`;
        ...
        console.error(`fetchWithAuth: Загальна помилка під час запиту до ${url}:`, error);
        throw error; // Прокидаємо помилку далі
    }
}
...
function showNewPlanNotificationModal() {
    return new Promise(resolve => {
        ...
        messageEl.innerHTML = "Чудові новини! ✨<br>Ми підготували для вас новий тренувальний план на наступні 30 днів. Ви можете знайти його у вкладці 'План'.";
        ...
        document.getElementById('new-plan-notification-ok-btn').addEventListener('click', closeModal);
...
function showDaySelectorModal(planToSchedule, userProfile) {
    return new Promise(resolve => {
        ...
        const previouslySelectedDays = userProfile.preferred_training_weekdays || [];
        ...
        const updateHint = () => {
            ...
            hint.style.color = (checkedCount !== expectedDaysCount && checkedCount > 0) ? '#ffc107' : '#aaa';
        };
        updateHint();
        ...
        confirmBtn.addEventListener('click', () => { // Забираємо async, він тут більше не потрібен
            const selectedDays = Array.from(grid.querySelectorAll('input:checked')).map(cb => cb.value);
            ...
            localStorage.setItem(`generation_in_progress_${planToSchedule.id}`, Date.now());

            if (selectedDays.length !== expectedDaysCount) {
                displayStatus('day-selector-status', `Будь ласка, оберіть рівно ${expectedDaysCount} дні(в).`, true, 4000);
                return;
            }
            ...
            `;
            document.getElementById('final-ok-btn').addEventListener('click', closeModal);

            // 2. Відправляємо запит на генерацію у фоновому режимі
            fetchWithAuth(`${baseURL}/api/plans/${planToSchedule.id}/schedule-and-generate`, {
                method: 'POST',
                ...
                .then(({ response, data }) => {
                    if (!response.ok) {
                        ...
                        console.error("Помилка фонової генерації тренувань:", data.detail || 'Невідома помилка');
                    } else {
                        console.log("Фонова генерація тренувань успішно запущена.");
                    }
                })
                .catch(error => {
                    console.error("Критична помилка під час запиту на фонову генерацію:", error);
                });
        });
...
function showCustomConfirmationDialog(message, onConfirm) {
    ...
    if (!overlay || !modal || !textElement || !yesButton || !noButton) {
        console.error("Елементи кастомного модального вікна не знайдені! Використовується стандартний confirm.");
        if (confirm(message)) {
            onConfirm();
        }
        return;
    ...
    document.getElementById('modalConfirmActionBtn').addEventListener('click', () => {
        onConfirm();
        closeModal();
    });

    document.getElementById('modalCancelActionBtn').addEventListener('click', closeModal);
...
async function preloadFirstActiveWorkoutGif() {
    ...
    try {
        ...
        const { data: plans, response: responsePlans } = await fetchWithAuth(`${baseURL}/training_plans`);

        if (!responsePlans.ok) {
            const errorText = plans?.detail || `Помилка сервера: ${responsePlans.status}`;
            console.warn(`Помилка отримання списку тренувань для передзавантаження: ${errorText}`);
            return;
        }
        ...
        const { data: planDetails, response: responseDetails } = await fetchWithAuth(`${baseURL}/training_plans/${planId}`);

        if (!responseDetails.ok) {
            const errorText = planDetails?.detail || `Помилка сервера: ${responseDetails.status}`;
            console.warn(`Помилка отримання деталей тренування ID ${planId} для передзавантаження: ${errorText}`);
            return;
        }
...
async function likeUserProfileAPI(targetUserPhone) {
    const { data, response } = await fetchWithAuth(`${baseURL}/community/users/${targetUserPhone}/like`, {
        method: 'POST',
    ...
    if (!response.ok) {
        ...
        throw new Error(data?.detail || `Помилка сервера: ${response.status} - ${response.statusText}`);
    }
...
async function loadAndDisplayNotifications() {
    ...
    try {
        const { data: notifications, response } = await fetchWithAuth(`${baseURL}/notifications`);
        if (!response.ok) {
            throw new Error(notifications.detail || 'Не вдалося завантажити повідомлення');
        }
        ...
        notifications.forEach(msg => {
            ...
            const date = new Date(msg.created_at).toLocaleString('uk-UA', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });
        ...
        if (unreadIds.length > 0) {
            ...
            }).catch(err => console.error("Не вдалося позначити повідомлення як прочитані:", err));

            setTimeout(() => {
                const unreadElements = container.querySelectorAll('.notification-item.is-unread');
...
function updateNotificationBadge(count) {
    const badge = document.querySelector('.tab-link[data-tab-name="notifications"] .notification-badge');
    if (!badge) return;
...
async function fetchAndDisplayUnreadCount() {
    ...
    if (!badge) {
        console.warn('Елемент для лічильника повідомлень (#notification-badge-id) не знайдено.');
        return;
    }
    ...
    try {
        ...
        const { data } = await fetchWithAuth(`${baseURL}/notifications/unread-count`);
        // ▲▲▲ КІНЕЦЬ ЗМІНИ ▲▲▲
        ...
        console.error("Помилка завантаження кількості непрочитаних повідомлень:", error);
...
function showReminderModal(daysLeft) {
    return new Promise(resolve => {
        ...
        okBtn.replaceWith(okBtn.cloneNode(true));
        document.getElementById('reminder-modal-ok-btn').addEventListener('click', closeModal);
        overlay.addEventListener('click', closeModal);
...
async function checkAndShowSubscriptionReminder() {
    ...
    try {
        ...
        if (currentUserProfileData && currentUserProfileData.auto_renew_enabled === true) {
            // console.log("Автопоновлення увімкнено, нагадування про закінчення підписки не потрібне.");
            return; // Виходимо з функції
        }

        const { data: subscriptions, response } = await fetchWithAuth(`${baseURL}/api/my-subscriptions`);
...
function translateField(field, value) {
    const translations = {
        subscription_type: { "weekly": "Тижнева", "monthly": "Місячна", "quarterly": "Квартальна (3 місяці)", "semi_annual": "Піврічна", "annual": "Річна" },
    };
...
function generatePaymentBlockHTML(userData, userSubscriptions) {
    ...
    const isFirstEverPayment = !userSubscriptions.some(sub => sub.status === 'active' || sub.status === 'expired');

    const hasPreviousPayment = userSubscriptions.some(sub => sub.plan_type === 'without_trainer' && sub.status === 'active');
    ...
    if (userData && userData.registration_type === 'self') {
        ...
        if (hasPreviousPayment) {
            plans = [
                { type: 'weekly', name: 'Тижнева', price: '250 грн', daily: '~35 грн/день' },
                { type: 'monthly', name: 'Місячна', price: '600 грн', daily: '~20 грн/день' },
                { type: 'quarterly', name: 'Квартальна', price: '1 500 грн', daily: '~16 грн/день' },
                { type: 'semi_annual', name: 'Піврічна', price: '2 700 грн', daily: '~15 грн/день' },
                { type: 'annual', name: 'Річна', price: '4 800 грн', daily: '~13 грн/день' }
            ];
        } else {
            plans = [
                { type: 'weekly', name: 'Тижнева', price: '99 грн', daily: '~14 грн/день', old_price: '250 грн', promo_text: 'Спробуйте лише за <strong>99 грн</strong> на тиждень — економте 50%!', is_discounted: true },
                { type: 'monthly', name: 'Місячна', price: '249 грн', daily: '~8 грн/день', old_price: '600 грн', promo_text: 'Отримайте місяць за <strong>249 грн</strong> — знижка 60%!', is_discounted: true },
                { type: 'quarterly', name: 'Квартальна', price: '1 500 грн', daily: '~16 грн/день' },
                { type: 'semi_annual', name: 'Піврічна', price: '2 700 грн', daily: '~15 грн/день' },
                { type: 'annual', name: 'Річна', price: '4 800 грн', daily: '~13 грн/день' }
            ];
        ...
        plans = [
            { type: 'weekly', name: 'Тижнева', price: '150 грн', daily: '~21 грн/день' },
            { type: 'monthly', name: 'Місячна', price: '400 грн', daily: '~13 грн/день' },
            { type: 'quarterly', name: 'Квартальна', price: '1 000 грн', daily: '~11 грн/день' },
            { type: 'semi_annual', name: 'Піврічна', price: '1 800 грн', daily: '~10 грн/день' },
            { type: 'annual', name: 'Річна', price: '3 400 грн', daily: '~9 грн/день' }
        ];
    }
    ...
    let introTextHTML = '';
    if (userData && userData.registration_type === 'self' && !hasPreviousPayment) {
...
async function handlePayment(event) {
    ...
    const autoRenewalCheckbox = document.getElementById('auto-renewal-agreement-checkbox');

    // 1. Перевірка згоди з умовами (залишається без змін)
    if (!agreementCheckbox || !agreementCheckbox.checked || (autoRenewalCheckbox && !autoRenewalCheckbox.checked)) {
        ...
        let errorMessage = 'Будь ласка, погодьтеся з умовами оферти та політики конфіденційності.';
        if (autoRenewalCheckbox && !autoRenewalCheckbox.checked) {
            errorMessage = 'Будь ласка, підтвердіть вашу згоду з автоподовженням підписки.';
        }
        ...
        if (agreementContainer) {
            ...
            setTimeout(() => { agreementContainer.classList.remove('highlight-error'); }, 1500);
        }
        return;
    }
    ...
    displayStatus('payment-status-message', `Готуємо оплату для підписки...`, false);

    try {
        // --- КРОК 1: АКТИВАЦІЯ АВТОПОНОВЛЕННЯ (ЯКЩО ПОТРІБНО) ---
        // Якщо чекбокс автопоновлення існує і він відмічений...
        if (autoRenewalCheckbox && autoRenewalCheckbox.checked) {
            displayStatus('payment-status-message', 'Активуємо автоподовження...', false);

            // ...ми відправляємо запит і ЧЕКАЄМО (await) на його завершення.
            const { response: autoRenewResponse, data: autoRenewData } = await fetchWithAuth(`${baseURL}/profile/auto-renew`, {
                method: 'PUT',
            ...
            if (!autoRenewResponse.ok) {
                throw new Error(autoRenewData.detail || 'Не вдалося налаштувати автоподовження.');
            }
        ...
        displayStatus('payment-status-message', 'Перенаправляємо на сторінку оплати...', false);
        const { data: paymentData, response } = await fetchWithAuth(`${baseURL}/api/initiate-payment`, {
            method: 'POST',
            body: JSON.stringify({
        ...
        if (!response.ok) {
            throw new Error(paymentData.detail || `Помилка сервера: ${response.status}`);
        }
        ...
        dataInput.type = 'hidden'; dataInput.name = 'data'; dataInput.value = paymentData.data;
        const signatureInput = document.createElement('input');
        signatureInput.type = 'hidden'; signatureInput.name = 'signature'; signatureInput.value = paymentData.signature;
        ...
        displayStatus('payment-status-message', `Помилка: ${error.message}`, true, 10000);
...
async function loadSubscriptionData() {
    ...
    try {
        ...
        const { data: subscriptions, response } = await fetchWithAuth(`${baseURL}/api/my-subscriptions`);
        if (!response.ok) { throw new Error(subscriptions.detail || `Помилка сервера: ${response.status}`); }
...
function displaySubscriptions(subscriptions, userData) {
    ...
    (subscriptions || []).forEach(sub => {
        ...
        if (sub.status === 'active' && endDate > new Date()) {
            if (!activeSubscription || endDate > new Date(activeSubscription.end_date)) {
    ...
    let finalHtml = '';
    const isFirstPaymentScenario = !activeSubscription && userData && userData.registration_type === 'self';
    ...
    if (!isFirstPaymentScenario) {
        ...
        if (activeSubscription) {
            const endDate = new Date(activeSubscription.end_date).toLocaleDateString('uk-UA', { day: '2-digit', month: '2-digit', year: 'numeric' });
            finalHtml += `
            ...
            finalHtml += '<p class="empty-section-message">У вас немає активної підписки. Будь ласка, оберіть тариф нижче, щоб продовжити користування всіма перевагами кабінету.</p>';
        }
        finalHtml += '</div>';
    }
    ...
    if (historySubscriptions.length > 0) {
        historySubscriptions.sort((a, b) => new Date(b.start_date) - new Date(a.start_date));
        finalHtml += '<div class="profile-section">';
        ...
        historySubscriptions.forEach(sub => {
            ...
            const statusTranslations = { 'expired': 'Закінчилась', 'cancelled': 'Скасована', 'failed': 'Не вдалася', 'pending_payment': 'Очікує оплати' };
            finalHtml += `
...
async function handleAutoRenewalToggle(event) {
    ...
    try {
        ...
        const { data, response } = await fetchWithAuth(`${baseURL}/profile/auto-renew`, {
            method: 'PUT',
...
async function checkInitialSubscriptionAndRedirect(forceRedirect = true) {
    ...
    try {
        ...
        if (forceRedirect && !hasActiveSub) {
            const subscriptionTabButton = document.querySelector('.tab-link[data-tab-name="subscription"]');
...
async function loadAndDisplayWorkoutPlans() {
    ...
    try {
        ...
        const { data: plans, response } = await fetchWithAuth(`${baseURL}/api/my-workout-plans`);
...
function renderWorkoutPlans(plans, hasActiveSubscription) {
    ...
    const activePlan = plans.find(p => new Date(p.start_date) <= today && new Date(p.end_date) >= today) || plans[0];
    const activePlanId = activePlan.id;

    plans.forEach((plan, index) => {
        const isActive = plan.id === activePlanId;
        const startDate = new Date(plan.start_date).toLocaleDateString('uk-UA', { day: '2-digit', month: '2-digit' });
        const endDate = new Date(plan.end_date).toLocaleDateString('uk-UA', { day: '2-digit', month: '2-digit', year: 'numeric' });
        ...
        subContentsHtml += `
            ...
            const typeClass = getWorkoutTypeClass(workout.title);
            return `
                                            <tr>
    ...
    if (subscriptionTabButton) {
        ...
        setTimeout(() => {
            ...
            if (subscriptionTabContent) {
                ...
                subscriptionTabContent.scrollIntoView({ behavior: 'smooth', block: 'start' });
            }
...
async function submitPlanEdit(planId) {
    const feedbackInput = document.getElementById(`plan-feedback-input-${planId}`);
    const statusDiv = document.getElementById(`edit-plan-status-${planId}`);
    const formContainer = document.getElementById(`edit-plan-container-${planId}`);
    const submitBtn = formContainer.querySelector('.main-action-button');
    const feedbackText = feedbackInput.value.trim();

    if (feedbackText.length < 10) {
        displayStatus(statusDiv.id, 'Будь ласка, опишіть ваші побажання більш детально (мінімум 10 символів).', true, 3000);
        return;
    }
    ...
    displayStatus(statusDiv.id, 'Відправляємо ваші правки та генеруємо новий план... Це може зайняти до 1 хвилини.', false);

    try {
        const { data: newPlan, response } = await fetchWithAuth(`${baseURL}/api/edit-workout-plan/${planId}`, {
            method: 'POST',
        ...
        displayStatus(statusDiv.id, 'Новий план успішно згенеровано! Оновлюємо...', false);
...
function openPlanSubTab(event, subTabContentId) {
    ...
    planContainer.querySelectorAll('.plan-sub-tabs .sub-tab-link').forEach(link => {
...
async function updatePlanTabVisibility() {
    const planTabButton = document.querySelector('.tab-link[data-tab-name="plan"]');
    if (!planTabButton) {
    ...
    try {
        ...
        if (currentUserProfileData && currentUserProfileData.registration_type === 'self') {
...
function initializeExcludedExercisesToggleForEditForm() {
    const toggleButton = document.getElementById('toggleExcludedExercisesBtn_edit'); // ID кнопки в формі редагування
    const checklistContainer = document.getElementById('excludedExercisesChecklistContainer_edit'); // ID контейнера в формі
    ...
    if (toggleButton && checklistContainer) {
        ...
        console.warn("Кнопка або контейнер для виключених вправ у формі редагування не знайдені.");
    }
}

async function loadAndRenderExcludedExercisesForEditForm() {
    const checklistContainer = document.getElementById('excludedExercisesChecklistContainer_edit');
    const statusElementId = 'profile-update-status_edit';

    if (!checklistContainer) {
        console.warn("Контейнер для виключених вправ у формі редагування не знайдено.");
        return;
    }
    ...
    checklistContainer.innerHTML = '<p>Завантаження доступних вправ для виключення...</p>';

    try {
        // Отримуємо доступні для виключення GIF
        const { data: availableGifObjects, response: availableResponse } = await fetchWithAuth(`${baseURL}/users/available-gif-names-for-exclusion`);
        if (!availableResponse.ok) {
            // У data може бути детальна помилка
            throw new Error(availableGifObjects?.detail || `Помилка завантаження доступних GIF: ${availableResponse.status}`);
        }

        const availableGifNames = availableGifObjects.map(gif => gif.name).filter(name => name && name.trim() !== "");

        // Отримуємо вже виключені користувачем GIF
        const { data: userExcludedGifNames, response: userExcludedResponse } = await fetchWithAuth(`${baseURL}/users/excluded-exercises`);
        if (!userExcludedResponse.ok) {
            throw new Error(userExcludedGifNames?.detail || `Помилка завантаження виключених GIF: ${userExcludedResponse.status}`);
        }

        renderExcludedExercisesChecklistInEditForm(availableGifNames, userExcludedGifNames, checklistContainer, statusElementId);

    } catch (error) {
        console.error('Помилка завантаження даних для виключених вправ (форма редагування):', error);
        checklistContainer.innerHTML = `<p style="color: red;">Не вдалося завантажити список: ${error.message}.</p>`;
    }
}

function renderExcludedExercisesChecklistInEditForm(availableNames, userExcludedNames, container, statusId) {
    container.innerHTML = ''; // Очищуємо контейнер

    if (!availableNames || availableNames.length === 0) {
        container.innerHTML = '<p>Ваш тренер ще не додав вправ, які можна було б виключити.</p>';
        return;
    }
    ...
    sortedAvailableNames.forEach((gifName, index) => {
        ...
        checkbox.addEventListener('change', async (event) => {
            ...
            try {
                ...
                displayStatus(statusId, `Налаштування для "${name}" ${isChecked ? 'додано' : 'видалено'}.`, false, 3000);

                // Оновлюємо клас для візуального відображення без перезавантаження
                label.classList.toggle('is-excluded', isChecked);

            } catch (error) {
                displayStatus(statusId, `Помилка для "${name}": ${error.message}`, true, 5000);
...
async function fetchCurrentProfileDataOnce() {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(data?.detail || `Помилка завантаження профілю: ${response.status}`);
        }
...
function displayUserProfileViewData(userData) {
    ...
    const emptyMessageContainer = document.getElementById('profile-view-empty-message');

    if (!profileViewDataContainer || !emptyMessageContainer) {
        console.error("Не знайдено DOM елементи #profile-view-data або #profile-view-empty-message.");
        return;
    }
    ...
    if (!userData) {
        ...
        if (profileViewSubTabToScroll) {
            profileViewSubTabToScroll.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
        return;
    }
    ...
    let generalInfoHtml = '<div class="profile-section profile-general-info-block">';
    ...
    } else { // Сюди потраплять всі, хто 'by_trainer'
    ...
    let preferencesHtml = '<div class="profile-section exercise-preferences-section">';
    preferencesHtml += `<h5 class="profile-section-title">Мої збережені показники</h5>`;

    if (userData.exercise_preferences_summary && userData.exercise_preferences_summary.length > 0) {
        preferencesHtml += `
...
function populateProfileEditForm(userData) {
    ...
    if (userData) { // Якщо є дані для заповнення
        if (statusDiv) displayStatus(statusDiv.id, "Дані завантажено для редагування.", false, 3000);
        ...
        const trainingLevelSelect = document.getElementById("level_of_training_edit");
        const healthProblemsSelect = document.getElementById("health_problems_edit");
        const otherHealthTextarea = document.getElementById("other_health_problems_edit");
        const excludedProductsSelect = document.getElementById("excluded_products_edit");
        const otherExcludedProductsTextarea = document.getElementById("other_excluded_products_edit");
        const mealsSelect = document.getElementById("number_of_meals_edit");
        const trainingDaysSelect = document.getElementById("training_days_per_week_edit");
        const weekdayCheckboxesContainer = document.getElementById("preferred_training_weekdays_edit_container");
        ...
        if (trainingTypeSelect) trainingTypeSelect.value = userData.type_of_training || "";
        if (trainingLevelSelect) trainingLevelSelect.value = userData.level_of_training || "";
        ...
        if (otherExcludedProductsTextarea) {
            otherExcludedProductsTextarea.value = userData.other_excluded_products || "";
            if (typeof autoResize === 'function') autoResize(otherExcludedProductsTextarea);
        }
        if (mealsSelect) mealsSelect.value = userData.number_of_meals || "";

        if (trainingDaysSelect) trainingDaysSelect.value = userData.training_days_per_week || "";

        const preferredWeekdays = userData.preferred_training_weekdays || [];
        if (weekdayCheckboxesContainer) {
            const weekdayCheckboxes = weekdayCheckboxesContainer.querySelectorAll('input[type="checkbox"]');
            weekdayCheckboxes.forEach(checkbox => {
                checkbox.checked = preferredWeekdays.includes(checkbox.value);
            });
        }

    } else { // Якщо userData null (профіль не існує)
        if (statusDiv) displayStatus(statusDiv.id, "Профіль ще не створено. Заповніть дані.", false);
...
async function loadAndDisplayUserProfileViewData() {
    ...
    if (statusDiv) displayStatus(statusDiv.id, "Завантаження даних профілю...", false);
    try {
        const profileData = await fetchCurrentProfileDataOnce();
        displayUserProfileViewData(profileData); // profileData може бути null
    } catch (error) {
        if (statusDiv) displayStatus(statusDiv.id, `Помилка: ${error.message}`, true);
        const emptyMessage = document.getElementById('profile-view-empty-message');
        const profileViewDataContainer = document.getElementById('profile-view-data');
...
async function loadProfileDataForEditForm() {
    const statusDiv = document.getElementById('profile-update-status_edit');
    if (statusDiv) displayStatus(statusDiv.id, "Завантаження даних для редагування...", false);
    try {
        const profileData = await fetchCurrentProfileDataOnce();
...
// Оновлення профілю
const updateProfileMainButton = document.getElementById("update-profile-main-btn"); // НОВИЙ ID
if (updateProfileMainButton) {
    updateProfileMainButton.addEventListener("click", async () => {
        ...
        const healthProblemsSelect = document.getElementById("health_problems_edit");
        const selectedHealthProblems = healthProblemsSelect ? Array.from(healthProblemsSelect.selectedOptions).map(option => option.value) : [];

        const excludedProductsSelect = document.getElementById("excluded_products_edit");
        const selectedExcludedProducts = excludedProductsSelect ? Array.from(excludedProductsSelect.selectedOptions).map(option => option.value) : [];
        const selectedWeekdays = Array.from(document.querySelectorAll('input[name="preferred_training_weekdays_edit"]:checked')).map(cb => cb.value);

        // Функція для отримання значень (використовуйте вашу, якщо є)
        const getEditInputValue = (idSuffix) => document.getElementById(idSuffix)?.value || null;
        const getEditIntValue = (idSuffix) => { const v = getEditInputValue(idSuffix); return v ? parseInt(v) : null; };
        const getEditFloatValue = (idSuffix) => { const v = getEditInputValue(idSuffix); return v ? parseFloat(v) : null; };

        // Валідація відповідності кількості днів та обраних днів
        const trainingDaysVal = getEditIntValue("training_days_per_week_edit");
        if (trainingDaysVal && selectedWeekdays.length > 0 && selectedWeekdays.length !== trainingDaysVal) {
            displayStatus(statusDiv.id, `Помилка: кількість обраних днів (${selectedWeekdays.length}) не відповідає вказаній кількості тренувань на тиждень (${trainingDaysVal}).`, true, 5000);
        ...
        const profileData = {
            ...
            health_problems: selectedHealthProblems.length > 0 ? selectedHealthProblems : null,
            other_health_problems: getEditInputValue("other_health_problems_edit"),
            excluded_products: selectedExcludedProducts.length > 0 ? selectedExcludedProducts : null,
            other_excluded_products: getEditInputValue("other_excluded_products_edit"),
            ...
            preferred_training_weekdays: selectedWeekdays.length > 0 ? selectedWeekdays : null
        };
        ...
        try {
            ...
            setTimeout(() => {
                const myProfileButton = document.querySelector('#profile .sub-tab-link[onclick*="profile-view"]');
                if (myProfileButton) {
...
function initializeWeekdaySelectorLogic() {
    const container = document.getElementById('preferred_training_weekdays_edit_container');
...
function initializeWeekdaysToggleForEditForm() {
    ...
    const checklistContainer = document.getElementById('preferred_training_weekdays_edit_container');
...
function initializePasswordChangeForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (newPassword.length < 8) {
            displayStatus(statusDivId, 'Новий пароль має містити щонайменше 8 символів.', true, 4000);
            return;
        }
        ...
        try {
            const { data, response } = await fetchWithAuth(`${baseURL}/profile/change-password`, {
                method: 'POST',
                body: JSON.stringify({
...
async function loadCommunityUsers(searchTerm = null) {
    const userListContainer = document.getElementById('community-user-list-container');
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(users?.detail || `Помилка завантаження списку: ${response.status}`);
        }
...
function renderCommunityUserList(users) {
    ...
    if (!users || users.length === 0) {
        userListContainer.innerHTML = '<p>Користувачів не знайдено або ще ніхто не зареєструвався.</p>';
        return;
    }
    ...
    users.sort((a, b) => {
        ...
        if (!a.is_suspended && b.is_suspended) return -1;
        const completedTrainingsDiff = (b.completed_trainings_count || 0) - (a.completed_trainings_count || 0);
    ...
    users.forEach(user => { // 'user' тут - це об'єкт типу CommunityUserListItem
        ...
        displayNameSpan.addEventListener('click', () => handleCommunityUserClick(user.display_name));
        ...
        totalLikesClickableSpan.className = 'community-user-total-likes clickable-likes'; // Додав .clickable-likes
        totalLikesClickableSpan.dataset.targetUserPhone = user.phone; // Потрібен телефон для API
        totalLikesClickableSpan.dataset.isLiked = user.is_liked_by_current_user.toString();
        totalLikesClickableSpan.dataset.currentTotal = user.total_likes_received || 0;
        ...
        totalLikesClickableSpan.title = user.is_liked_by_current_user ? 'Зняти лайк' : 'Поставити лайк';
...
async function displayPublicUserProfile(displayName) {
    const profileDisplayContainer = document.getElementById('community-selected-user-profile');
    ...
    try {
        const { data: userData, response } = await fetchWithAuth(`${baseURL}/community/users/${encodeURIComponent(displayName)}`);
        if (!response.ok) {
        ...
        if (profileDisplayContainer.scrollIntoView) {
            profileDisplayContainer.scrollIntoView({ behavior: 'smooth', block: 'start' });
        } else { // Fallback для старих браузерів
            profileDisplayContainer.scrollTop = 0;
        }

    } catch (error) {
        console.error(`Помилка завантаження публічного профілю для ${displayName}:`, error);
...
function renderPublicUserProfile(userData, container) {
    ...
    const registrationDateText = userData.registration_date ? new Date(userData.registration_date).toLocaleDateString('uk-UA') : 'Не вказано';
    ...
    if (userData.instagram_link || userData.telegram_link) { // Показуємо блок, якщо є хоча б одне посилання
    ...
    if (userData.progress_list && userData.progress_list.length > 0) {
        ...
        userData.progress_list.forEach(p => {
            const displayWeight = (p.weight !== null && p.weight !== undefined) ? p.weight : '-';
            const displayChest = (p.chest !== null && p.chest !== undefined) ? p.chest : '-';
            const displayWaist = (p.waist !== null && p.waist !== undefined) ? p.waist : '-';
            const displayAbdomen = (p.abdomen !== null && p.abdomen !== undefined) ? p.abdomen : '-';
            const displayHips = (p.hips !== null && p.hips !== undefined) ? p.hips : '-';
    ...
    let preferencesHtmlString = '<div class="profile-section exercise-preferences-section">';
...
function generatePublicPreferenceTableHTML(preferenceItem) {
    ...
    const showWeightsColumn = weights.some(val => val !== null && val !== undefined);
    ...
    if (showWeightsColumn) tableHeaderHTML += '<th>Вага <span class="unit-label-table">(кг)</span></th>';
    if (showTimeColumn) tableHeaderHTML += '<th>Час <span class="unit-label-table">(сек)</span></th>';
    ...
    for (let i = 0; i < numSets; i++) {
        ...
        if (showWeightsColumn) {
            const weightVal = (weights[i] !== null && weights[i] !== undefined) ? weights[i] : '-';
...
function scrollPreferencesCarousel(button, direction) {
    ...
    const viewport = carouselContainer.querySelector('.preferences-carousel-viewport');
    if (!viewport) return;
    ...
    const slideMarginRight = parseFloat(window.getComputedStyle(firstSlide).marginRight) || 0;
...
function updateCarouselButtonsState(viewport) {
    ...
    const prevButton = viewport.closest('.preferences-carousel-container').querySelector('.carousel-arrow.prev');
    const nextButton = viewport.closest('.preferences-carousel-container').querySelector('.carousel-arrow.next');

    if (!track || !prevButton || !nextButton) return;

    prevButton.disabled = viewport.scrollLeft <= 0;
    nextButton.disabled = viewport.scrollLeft + viewport.offsetWidth >= track.scrollWidth - 5; // -5 для невеликої похибки
}

// Загрузка данных прогресса
async function loadProgressData() {
    const tableBody = document.getElementById("progress-table");
    const statusDiv = document.getElementById('progress-add-status'); // Використаємо цей div для статусу завантаження

    if (!tableBody) return; // Виходимо, якщо таблиця не знайдена
    tableBody.innerHTML = ""; // Очищуємо таблицю
    if (statusDiv) statusDiv.innerText = "Завантаження прогресу...";
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(data?.detail || `Помилка завантаження: ${response.status}`);
        }

        if (statusDiv) statusDiv.innerText = "";

        if (data.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="6">Записи прогресу відсутні.</td></tr>';
        } else {
...
if (addProgressButton) {
    addProgressButton.addEventListener("click", async () => {
        ...
        if (!weight && !chest && !waist && !abdomen && !hips) {
            if (statusDiv) statusDiv.innerText = "Будь ласка, заповніть хоча б одне поле прогресу.";
            return;
        }
...
function openRatingsSubTab(event, subTabContentId) {
    ...
    ratingsContainer.querySelectorAll('.leaderboard-sub-content').forEach(content => {
        content.style.display = 'none';
        content.classList.remove('active-sub-content');
    });

    ratingsContainer.querySelectorAll('.ratings-sub-tabs .sub-tab-link').forEach(link => {
        link.classList.remove('active');
    });
 ...
 */
async function loadLeaderboardData(endpoint, containerId, title, renderFunction, limit) {
...
function loadTotalTrainingsLeaderboard() {
    loadLeaderboardData('/leaderboard/completed-trainings', 'leaderboard-total', '"Тільки системний підхід дасть справжній результат! 🏆" - Tоп за кількістю тренувань (за весь час)', renderTrainingsLeaderboard, 50);
}
function loadWeeklyTrainingsLeaderboard() {
    loadLeaderboardData('/leaderboard/completed-trainings-weekly', 'leaderboard-weekly', '"Наполегливість вирішує! 💎" - Tоп за кількістю тренувань (за останні 7 днів)', renderTrainingsLeaderboard, 50);
}
function loadWeightLossLeaderboard() {
    loadLeaderboardData('/leaderboard/weight-loss', 'leaderboard-weight-loss', 'Топ за втратою ваги (мета "Схуднути")', renderPerformanceLeaderboard, 50);
}
function loadWeightGainLeaderboard() {
    loadLeaderboardData('/leaderboard/weight-gain', 'leaderboard-weight-gain', 'Топ за набором ваги (мета "Набрати м\'язову масу")', renderPerformanceLeaderboard, 50);
}
function loadHipThrustLeaderboard() {
    loadLeaderboardData('/leaderboard/exercise/hip-thrust', 'leaderboard-hip-thrust', 'Топ у вправі "Сідничний міст" (за макс. вагою в "кг")', renderPerformanceLeaderboard, 50);
}
function loadPullUpsLeaderboard() {
    loadLeaderboardData('/leaderboard/exercise/pull-ups', 'leaderboard-pull-ups', 'Топ у вправі "Підтягування широким хватом" (за макс. повтореннями)', renderPerformanceLeaderboard, 50);
}

/**
 * ОНОВЛЕНО: Відображає реальну кількість користувачів без порожніх рядків.
 */
function renderTrainingsLeaderboard(leaderboardData, listElement, statusElement, limit) {
    ...
    leaderboardData.forEach(entry => {
        ...
        totalLikesClickableSpan.dataset.isLiked = entry.is_liked_by_current_user.toString();
        totalLikesClickableSpan.dataset.currentTotal = entry.total_likes_received || 0;
        ...
        totalLikesClickableSpan.appendChild(heartIconInTotal);
        totalLikesClickableSpan.appendChild(countNumberSpan);
        totalLikesClickableSpan.title = entry.is_liked_by_current_user ? 'Зняти лайк' : 'Поставити лайк';
 ...
 */
function renderPerformanceLeaderboard(leaderboardData, listElement, statusElement, limit) {
    listElement.innerHTML = '';
    statusElement.textContent = '';
 ...
 */
async function handleGenericListLikeClick(buttonElement, context) { // context поки залишаємо, може знадобитися
    const targetUserPhone = buttonElement.dataset.targetUserPhone;
    let currentlyLiked = buttonElement.dataset.isLiked === 'true';
    let currentTotalOnElement = parseInt(buttonElement.dataset.currentTotal || "0");
    ...
    if (!targetUserPhone) {
        console.error("Не вдалося отримати телефон цільового користувача для лайка.");
        return;
    }
    ...
    try {
        ...
        console.error(`Помилка при ${currentlyLiked ? 'знятті' : 'встановленні'} лайка (${context}):`, error);
...
async function showUserProfileFromAnywhere(displayName) {
    ...
    const profileTabButton = Array.from(document.querySelectorAll('.tab-link'))
        .find(btn => btn.getAttribute('onclick')?.includes("openTab(event, 'profile')"));
    if (profileTabButton) {
    ...
    const communitySubTabButton = Array.from(document.querySelectorAll('#profile .sub-tab-link'))
        .find(btn => btn.getAttribute('onclick')?.includes("openProfileSubTab(event, 'profile-community')"));
    if (communitySubTabButton) {
        ...
        openProfileSubTab({ currentTarget: communitySubTabButton }, 'profile-community');
    } else {
...
async function checkAndDisplayIndependentFeatures() {
    const addWorkoutBtn = document.getElementById('show-independent-workout-form-btn');
...
async function loadWorkoutList(isLoadMore = false) {
    ...
    const loadMoreContainer = document.getElementById('workout-load-more-container');

    // ... (код на початку функції залишається без змін) ...
    ...
    if (!isLoadMore) {
        ...
        currentUserWorkoutPage = 0;
        if (!currentUserProfileData) {
            currentUserProfileData = await fetchCurrentProfileDataOnce().catch(() => null);
        }
    }
    ...
    try {
        ...
        const { data: plans, response } = await fetchWithAuth(`${baseURL}/training_plans?skip=${skip}&limit=${limit}`);
        if (!response.ok) throw new Error(plans.detail || `Помилка сервера: ${response.status}`);
        ...
        if (plans && plans.length > 0) {
            plans.forEach(plan => {
                ...
                if (localStorage.getItem(flagKey)) {
                    console.log(`Тренування для плану ${plan.id} завантажено. Видаляємо прапорець генерації.`);
        ...
        if (!isLoadMore && (!plans || plans.length === 0)) {
            workoutListContainer.innerHTML = '<p>У вас ще немає призначених тренувань.</p>';
            return;
        }
        if (response.headers.has('x-total-count')) {
            totalUserWorkoutsAvailable = parseInt(response.headers.get('x-total-count'), 10);
        }
        ...
        plans.forEach(plan => {
            ...
            const planDateFormatted = !isNaN(planDateObj) ? planDateObj.toLocaleDateString('uk-UA', { year: 'numeric', month: 'long', day: 'numeric' }) : 'Невідома дата';
            const planWeekday = !isNaN(planDateObj) ? planDateObj.toLocaleDateString('uk-UA', { weekday: 'short' }) : '';
            listItem.innerHTML = `
            ...
            listItem.addEventListener('click', (e) => {
                if (e.target.closest('.duplicate-workout-btn') || e.target.closest('.edit-workout-btn')) return;
            ...
            if (currentUserProfileData?.is_independent) {
                if (plan.completed) {
                    ...
                    duplicateBtn.addEventListener('click', () => handleDuplicateWorkout(plan.id));
 ...
 * @returns {string} HTML рядок таблиці.
 */
function generateUserReadOnlyTableHTML(plannedReps, plannedWeights, plannedTime, completedReps, completedWeights, completedTime) {
    ...
    const numRows = Math.max(
        plannedReps.length, plannedWeights.length, plannedTime.length,
        completedReps.length, completedWeights.length, completedTime.length
    );
    ...
    const hasAnyData = (arrValues) => Array.isArray(arrValues) && arrValues.some(val => val !== null && val !== undefined);

    const showRepsColumns = hasAnyData(plannedReps) || hasAnyData(completedReps);
    const showWeightsColumns = hasAnyData(plannedWeights) || hasAnyData(completedWeights);
    ...
    let tableHeaderHTML = '<th>Підхід</th>';
    if (showRepsColumns) {
        tableHeaderHTML += '<th>Повт. (план)</th><th class="user-fact-col">Повт. (факт)</th>';
    }
    if (showWeightsColumns) {
        tableHeaderHTML += '<th>Вага (план)</th><th class="user-fact-col">Вага (факт)</th>';
    }
    if (showTimeColumns) {
        tableHeaderHTML += '<th>Час (план)</th><th class="user-fact-col">Час (факт)</th>';
    }
    ...
    for (let i = 0; i < numRows; i++) {
        ...
        if (showRepsColumns) {
            ...
            const planRepDisplay = (planRepValue !== null && planRepValue !== undefined) ? planRepValue : '--';
            const factRepDisplay = (factRepValue !== null && factRepValue !== undefined) ? factRepValue : '--';
        ...
        if (showWeightsColumns) {
            ...
            const planWeightDisplay = (planWeightValue !== null && planWeightValue !== undefined)
                ? `${planWeightValue} кг` // Додаємо " кг"
                : '--';

            // ФОРМУЄМО РЯДОК ДЛЯ ВІДОБРАЖЕННЯ ФАКТИЧНОЇ ВАГИ З ОДИНИЦЯМИ "кг"
            const factWeightDisplay = (factWeightValue !== null && factWeightValue !== undefined)
        ...
        if (showTimeColumns) {
            ...
            const planTimeDisplay = (planTimeValue !== null && planTimeValue !== undefined) ? `${planTimeValue} сек` : '--';
            const factTimeDisplay = (factTimeValue !== null && factTimeValue !== undefined) ? `${factTimeValue} сек` : '--';
...
async function showWorkoutDetails(planId) {
    ...
    const exercisesContainer = document.getElementById('workout-details-exercises');
    ...
    if (!detailsContainer || !listContainer || !detailsStatusDiv || !exercisesContainer) {
        console.error("Критична помилка: Не знайдено основні контейнери для деталей тренування.");
        return;
    }
    ...
    displayStatus('workout-details-status', 'Завантаження деталей тренування...', false);
    ...
    try {
        ...
        try {
            const { data: userExcludedGifNames, response: excludedResponse } = await fetchWithAuth(`${baseURL}/users/excluded-exercises`);
            if (excludedResponse.ok) {
                userExcludedGifNamesCurrent = userExcludedGifNames || [];
            }
        } catch (exError) {
            console.warn("[showWorkoutDetails] Помилка при завантаженні списку виключених вправ:", exError);
        }

        const { data: plan, response } = await fetchWithAuth(`${baseURL}/training_plans/${planId}`);
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ detail: 'Не вдалося розпарсити помилку сервера.' }));
            throw new Error(errorData.detail || `Помилка сервера: ${response.status}`);
        }

        const completedExercisesDataKey = `completedPlan_${planId}`;
        const completedExercisesMap = JSON.parse(localStorage.getItem(completedExercisesDataKey)) || {};
        ...
        detailsDesc.innerHTML = formatTextWithLineBreaks(plan.description) || 'Загальний опис до тренування відсутній, схоже твій тренер вважає тебе професіоналом 💪';
        ...
        if (plan.exercises && Array.isArray(plan.exercises) && plan.exercises.length > 0) {
            plan.exercises.sort((a, b) => a.order - b.order).forEach(exercise => {
                ...
                const isEffectivelyExcluded = exercise.is_excluded_by_user === true || (gifName && userExcludedGifNamesCurrent.includes(gifName));
                ...
                if (exerciseGif && typeof exerciseGif.filename === 'string' && exerciseGif.filename.trim() !== '') {
                ...
                slowLoadMessageDiv.textContent = 'Поганий інтернет зв\'язок, іде завантаження анімації вправи';
                slowLoadMessageDiv.style.display = 'none';
                mediaContainer.appendChild(slowLoadMessageDiv);

                const gifImg = document.createElement('img');
                if (gifUrl) {
                    gifImg.alt = exerciseGif ? (exerciseGif.name || 'Анімація вправи') : 'Анімація вправи';
                    ...
                    let slowLoadTimer = setTimeout(() => {
                        if (gifImg.style.display === 'none' && !gifImg.dataset.loadError && gifImg.src) {
                    ...
                    gifImg.onload = () => {
                        ...
                        if (pngImg.style.display !== 'none') {
                            pngImg.style.display = 'none';
                            const previewErrorMsg = mediaContainer.querySelector('.preview-error-message');
                    ...
                    loaderDiv.style.display = 'none';
                    if (pngImg.style.display === 'none' && !mediaContainer.querySelector('.preview-error-message')) {
                        if (!mediaContainer.querySelector('.no-media-message')) {
                            ...
                            noMediaP.style.cssText = 'color: #888; font-size: 0.8em; text-align: center;';
                ...
                descriptionText.innerHTML = formatTextWithLineBreaks(exercise.gif.description) || 'Опис відсутній.';
                ...
                if (exercise.superset) {
                    ...
                    span.innerHTML = '<strong>Виконувати в суперсеті з наступною вправою ⇓</strong>';
                    detailsContentDiv.appendChild(span);
                }
                ...
                exerciseDiv.dataset.plannedWeights = JSON.stringify(exercise.weights || []);
                ...
                if (isCompleted) {
                    ...
                    if (completedData && Array.isArray(completedData.completedReps) && Array.isArray(completedData.completedWeights)) {
                        ...
                        setsTableContainer.innerHTML = '<p style="color: orange;">Помилка: Збережені дані про виконання некоректні.</p>';
                    }
                } else {
                    setsTableContainer.innerHTML = generateEditableSetsTableHTML(exercise);
                }
                ...
                // === ТАЙМЕР ===                
                if (Array.isArray(exercise.time) && exercise.time.some(t => t !== null && t > 0)) {
                    ...
                    addSafeEventListener(startWorkBtn, async () => {
                        ...
                        const workDurations = exercise.time.filter(t => t !== null && t > 0);
                ...
                if (!isCompleted) {
                    ...
                    saveButton.title = 'Позначити вправу виконаною та зберегти показники'; // Оновлено title
            ...
            plan.exercises.forEach(exercise => {
                ...
                if (exercise.order > 1 && exercise.gif && exercise.gif.filename) {
                    ...
                    imgPreloader.onerror = () => {
                        console.warn(`Помилка фонового завантаження GIF: ${subsequentGifUrl}`);
                    };
                    imgPreloader.onload = () => {
            ...
            console.warn('Масив plan.exercises відсутній, порожній або не є масивом.');
            exercisesContainer.innerHTML = '<p>Вправи для цього тренування ще не додані.</p>';
        }

        // Оновлюємо вигляд елемента у списку тренувань, якщо потрібно
        updateWorkoutListItemAppearance(planId, planContainsExcludedExerciseInitially);
        ...
        const exercisesContainerElement = document.getElementById('workout-details-exercises');
        if (exercisesContainerElement) {
            ...
            const completeButton = completionDiv.querySelector('#complete-workout-button');
            ...
            if (completeButton) {
                completeButton.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        const { data: updatedPlan, response: patchResponse } = await fetchWithAuth(`${baseURL}/training_plans/${planIdToUpdate}/status`, {
                            method: 'PATCH',
                            body: JSON.stringify({ completed: newStatus })
                        });

                        if (!patchResponse.ok) {
                            throw new Error(errorData.detail || `Помилка сервера: ${patchResponse.status}`);
                        }

                        // Оновлюємо UI на основі відповіді сервера
                        completeButton.textContent = updatedPlan.completed ? '✔ Виконано' : 'Позначити тренування виконаним';
                        completeButton.classList.toggle('completed', updatedPlan.completed);
                        completeButton.classList.toggle('not-completed', !updatedPlan.completed);

                        // ОНОВЛЕНО: Керуємо видимістю ОБОХ текстових повідомлень
                        const greetingTextDiv = document.getElementById('greeting-text');
                        const warningTextDiv = document.getElementById('uncompleted-warning-text');

                        if (greetingTextDiv) {
                            greetingTextDiv.style.display = updatedPlan.completed ? 'block' : 'none';
                        }
                        if (warningTextDiv) {
                            warningTextDiv.style.display = updatedPlan.completed ? 'none' : 'block';
                        }

                        // --- ДОДАЄМО ОНОВЛЕННЯ ЕЛЕМЕНТА У СПИСКУ ---
                        const listItemInList = document.querySelector(`.workout-list-item[data-plan-id="${planIdToUpdate}"]`);
                        if (listItemInList) {
                            ...
                            console.warn(`Елемент списку для плану ${planIdToUpdate} не знайдено для оновлення.`);
                        }

                    } catch (error) {
                        console.error("Помилка оновлення статусу тренування:", error);
                        if (typeof displayStatus === 'function') {
                            displayStatus(statusElementId, `Помилка: ${error.message}`, true, 5000);
                        } else {
            ...
            console.error("Елемент #workout-details-exercises не знайдено для вставки кнопки виконання.");
        }
        // --- КІНЕЦЬ ДОДАВАННЯ КНОПКИ ---

    } catch (error) { // Обробка помилок fetch або парсингу JSON
...
async function handleFeedbackAndAIAnalysis(event) {
    ...
    const feedbackData = {
        training_duration: parseInt(document.getElementById('workout-feedback-duration')?.value) || null,
        cardio_duration: parseInt(document.getElementById('workout-feedback-cardio')?.value) || null,
        calories_burned: parseInt(document.getElementById('workout-feedback-calories')?.value) || null,
        difficulty: document.getElementById('workout-feedback-difficulty')?.value || null,
    ...
    displayStatus(feedbackStatusDiv.id, 'Ваш відгук збережено. Запускаємо аналіз від Gemini... Це може зайняти до хвилини.', false);
    ...
    try {
        ...
        fetchWithAuth(`${baseURL}/training_plans/${planId}/ai-feedback`, { method: "POST" });
        ...
        displayStatus(feedbackStatusDiv.id, `Помилка збереження відгуку: ${error.message}`, true, 10000);
...
/**
 * Періодично перевіряє, чи готовий аналіз від Gemini для вказаного тренування.
 * @param {number} planId - ID тренування, яке перевіряється.
 */
function startPollingForAIAnalysis(planId) {
    ...
    aiAnalysisPollInterval = setInterval(async () => {
        if (attempts >= maxAttempts) {
            clearInterval(aiAnalysisPollInterval);
            console.warn(`Зупинено перевірку для плану ${planId}: перевищено ліміт спроб.`);
            displayStatus('feedback-status', 'Не вдалося автоматично завантажити аналіз. Оновіть сторінку.', true, 10000);
            return;
        }

        try {
            // Тихо запитуємо дані тренування, без показу "Завантаження..."
            const { data: plan, response } = await fetchWithAuth(`${baseURL}/training_plans/${planId}`);
            ...
            if (response.ok && plan && plan.ai_feedback_analysis) {
                ...
                clearInterval(aiAnalysisPollInterval); // Зупиняємо перевірку

                // Оновлюємо всю секцію фідбеку, показуючи готовий результат
                renderFeedbackSection(plan);
            }
        } catch (error) {
            console.error(`Помилка під час перевірки аналізу для плану ${planId}:`, error);
            // Не показуємо помилку користувачу, просто спробуємо ще раз
        }
...
function renderFeedbackSection(plan) {
    ...
    const isConcisePreferred = currentUserProfileData?.prefers_concise_analysis === true;
...
async function updateConcisePreference(isConcise) {
    try {
        ...
        const statusText = isConcise ? "Наступний аналіз буде стислим." : "Наступний аналіз буде повним.";
...
function initializeExcludeExerciseButtons(currentPlanId, currentUserExcludedNames) {
    const excludeButtons = document.querySelectorAll('#workout-details-exercises .exclude-exercise-btn');
    excludeButtons.forEach(button => {
        ...
        button.addEventListener('click', (event) => {
            ...
            showCustomConfirmationDialog(confirmationText, async () => {
                displayStatus('workout-details-status', `Виключення вправи "${gifName}"...`, false);
                try {
                    const { data: updatedExclusions, response } = await fetchWithAuth(`${baseURL}/users/excluded-exercises`, {
                        method: 'POST',
                    ...
                    if (!response.ok) {
                        ...
                        throw new Error(updatedExclusions?.detail || `Помилка сервера: ${response.status}`);
                    }

                    displayStatus('workout-details-status', `Вправа "${gifName}" додана до виключених.`, false, 3000);
                    ...
                    displayStatus('workout-details-status', `Помилка виключення "${gifName}": ${error.message}`, true, 5000);
                }
            });
        });
    });
...
function updateWorkoutListItemAppearance(planId, containsExcluded) {
    const workoutListItem = document.querySelector(`.workout-list-item[data-plan-id="${planId}"]`);
    if (workoutListItem) {
        if (containsExcluded) {
...
const FOLDER_TRANSLATIONS = {
    gym: 'Зал', home: 'Дім', street: 'Вулиця', trx: 'TRX', arms: 'Руки',
    abs: 'Прес', delts: 'Дельти', legs: 'Ноги', chest: 'Груди',
    back: 'Спина', functional: 'Функціонал', resistance_band: 'Гумові петлі'
};
...
async function loadAndApplyUserWorkoutDraft() {
    ...
    if (savedDraftJSON) {
        try {
            ...
            displayStatus('user-training-plan-message', 'Знайдено та завантажено попередню чернетку.', false, 3000);
        } catch (e) {
...
async function setupUserWorkoutForm(planData = null) {
    ...
    const toggleGeminiBtn = document.getElementById('user-toggle-gemini-helper-btn');
    const geminiInputSection = document.getElementById('user-gemini-input-section');
    const generateWithGeminiBtn = document.getElementById('user-generate-with-gemini-btn');
    ...
    document.getElementById('user-add-exercise-btn').addEventListener('click', handleUserAddExercise);
    document.getElementById('user-back-to-list-btn').addEventListener('click', () => showUserWorkoutView('list'));
    document.getElementById('independent-workout-form').addEventListener('submit', handleUserTrainingPlanSubmit);
    document.getElementById('user-clear-workout-draft-btn').addEventListener('click', async () => {
    ...
    if (!userGifsCache['all']?.length) {
        displayStatus('user-training-plan-message', 'Завантаження ресурсів для вправ...');
    ...
    if (planData) {
        ...
        form.elements['training-date'].value = planData.date ? new Date(planData.date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0];
        form.elements['training-description'].value = planData.description || '';
        if (planData.exercises?.length) {
            userExerciseCounter = 0;
            for (const exercise of planData.exercises.sort((a, b) => a.order - b.order)) {
                const usePrefs = !planData.id;
                await addUserExerciseToFormWithData(exercise, usePrefs);
            }
        }
    } else {
        form.elements['training-date'].value = new Date().toISOString().split('T')[0];
        form.elements['training-description'].value = 'Моє самостійне тренування. 💪';
        userExerciseCounter = 0;
    }
...
async function addUserExerciseToFormWithData(exerciseData, usePreferences = false) {
    userExerciseCounter++;
    const exerciseFieldset = createExerciseFieldsetHTML(userExerciseCounter);
    document.getElementById('user-exercises-container').appendChild(exerciseFieldset);
    exerciseFieldset.querySelector('.order-input').value = exerciseData.order || userExerciseCounter;
    const gifId = exerciseData.gif_id;
    if (gifId) {
        const gifData = (userGifsCache['all'] || []).find(g => g.id == gifId);
        if (gifData) {
            await selectGifForUserExercise(exerciseFieldset, gifData, usePreferences, false);
        }
    }

    exerciseFieldset.querySelector('.superset-input').checked = exerciseData.superset || false;
    exerciseFieldset.querySelector('.emphasis-input').checked = exerciseData.emphasis || false;
    exerciseFieldset.querySelector('.total-weight-input').checked = exerciseData.total_weight || false;
    exerciseFieldset.querySelector('.total-reps-input').checked = exerciseData.total_reps || false;

    if (exerciseData.rest_time) {
        const totalSeconds = exerciseData.rest_time;
        exerciseFieldset.querySelector('.rest-time-minutes').value = Math.floor(totalSeconds / 60);
        exerciseFieldset.querySelector('.rest-time-seconds').value = totalSeconds % 60;
    }

    if (!usePreferences) {
        const setsInput = exerciseFieldset.querySelector(".sets-input");
        const setsTableContainer = exerciseFieldset.querySelector(".sets-table-container");
        ...
        const weightsSelects = setsTableContainer.querySelectorAll('.weight-select');
        const timeSelects = setsTableContainer.querySelectorAll('.time-select');
        for (let i = 0; i < numSets; i++) {
            if (repsSelects[i] && exerciseData.reps) repsSelects[i].value = exerciseData.reps[i] || "";
            if (weightsSelects[i] && exerciseData.weights) weightsSelects[i].value = exerciseData.weights[i] || "";
            if (timeSelects[i] && exerciseData.time) timeSelects[i].value = exerciseData.time[i] || "";
        }
    }
...
function createExerciseFieldsetHTML(counter) {
    ...
    deleteBtn.type = 'button'; deleteBtn.innerHTML = '❌'; deleteBtn.className = 'delete-exercise-btn'; deleteBtn.title = 'Видалити цю вправу';
    deleteBtn.onclick = () => {
        if (confirm("Видалити цю вправу?")) {
            exerciseFieldset.remove();
            document.querySelectorAll('#user-exercises-container .exercise-number').forEach((span, idx) => {
            ...
            userExerciseCounter = document.getElementById('user-exercises-container').children.length;
            saveUserWorkoutDraft();
        }
    };
    exerciseFieldset.appendChild(deleteBtn);

    exerciseFieldset.querySelector(".select-gif-btn").addEventListener("click", () => handleUserGifSelection(exerciseFieldset));
    exerciseFieldset.querySelector(".change-gif-btn").addEventListener("click", () => handleUserGifSelection(exerciseFieldset));
    exerciseFieldset.querySelector(".sets-input").addEventListener("change", (e) => {
        generateUserEditableSetsTable(parseInt(e.target.value) || 0, exerciseFieldset.querySelector(".sets-table-container"));
        saveUserWorkoutDraft();
    });
...
async function handleUserGifSelection(exerciseFieldset) {
    const selectorContainer = exerciseFieldset.querySelector('.gif-selector-container');
    ...
    const isVisible = window.getComputedStyle(selectorContainer).display === 'block';
    ...
    const detailsWrapper = exerciseFieldset.querySelector('.exercise-details-wrapper');
    if (detailsWrapper) {
    ...
    if (!isChangingGif) {
        const selectedGifContainer = exerciseFieldset.querySelector('.selected-gif-container');
        if (selectedGifContainer) selectedGifContainer.style.display = 'none';

        const techniqueToggle = exerciseFieldset.querySelector('.technique-toggle');
        if (techniqueToggle) techniqueToggle.style.display = 'none';

        const techniqueContent = exerciseFieldset.querySelector('.technique-content');
    ...
    const subfolderSelector = exerciseFieldset.querySelector('.subfolder-selector');
    ...
    const folderButtonsContainer = folderSelector.querySelector('.folder-buttons');
    folderButtonsContainer.innerHTML = folders.map(f =>
    ...
    folderButtonsContainer.querySelectorAll('.folder-btn').forEach(button => {
        button.addEventListener('click', () => {
            ...
            folderButtonsContainer.querySelectorAll('.folder-btn').forEach(btn => btn.classList.remove('active'));
            button.classList.add('active');

            const subfolders = getUniqueSubfolders(gifs, folder);
            const subfolderButtonsContainer = subfolderSelector.querySelector(".subfolder-buttons");
            ...
            if (subfolders.length === 0) {
                ...
                const gifsForDisplay = gifs.filter(gif => gif.filename.startsWith(prefix) && !gif.filename.substring(prefix.length).includes('/'));
                displayGifsForUser(gifsForDisplay, exerciseFieldset);
            } else {
                gifGrid.style.display = "none";
                subfolderSelector.style.display = "block";
                subfolderButtonsContainer.innerHTML = subfolders.map(sub => `<button type="button" class="subfolder-btn" data-subfolder="${sub}">${translateFolderName(sub)}</button>`).join('');

                subfolderButtonsContainer.querySelectorAll('.subfolder-btn').forEach(subButton => {
                    subButton.addEventListener('click', () => {
                        const subfolder = subButton.dataset.subfolder;
                        subfolderButtonsContainer.querySelectorAll('.subfolder-btn').forEach(btn => btn.classList.remove('active'));
                        subButton.classList.add('active');
                        const activeFolderButton = folderSelector.querySelector(".folder-btn.active");
                        if (!activeFolderButton) return;
                        const folder = activeFolderButton.dataset.folder;
                        const creatorPhone = getGifCreatorPhone(currentUserProfileData);
                        const prefix = `${creatorPhone}/${folder}/${subfolder}/`;
                        const gifsForDisplay = gifs.filter(gif => gif.filename.startsWith(prefix));
                        gifGrid.style.display = "grid";
                        displayGifsForUser(gifsForDisplay, exerciseFieldset);
                    });
                });
...
function displayGifsForUser(gifs, exerciseFieldset) {
    ...
    gifs.forEach(gif => {
        ...
        gifItem.title = 'Натисніть для перегляду анімації, натисніть ще раз для вибору';

        const previewImg = document.createElement("img");
        previewImg.className = 'gif-preview';
        const pngFilename = (gif.filename.substring(0, gif.filename.lastIndexOf('.')) || gif.filename) + '.png';
        ...
        gifItem.addEventListener("click", () => {
            ...
            if (gifItem.classList.contains('is-loading') || gifItem.classList.contains('is-loaded')) {
...
function getGifCreatorPhone(profileData) {
    ...
    console.error("Не вдалося визначити телефон творця GIF з даних профілю:", profileData);
    return null;
}
 ...
 */
async function selectGifForUserExercise(fieldset, gif, loadPreferences = true, shouldScroll = false) {
    ...
    const selectedGifContainer = fieldset.querySelector('.selected-gif-container');
    ...
    finalGifImg.onload = () => { loader.style.display = 'none'; previewImg.style.display = 'none'; finalGifImg.style.display = 'block'; };
    finalGifImg.onerror = () => { console.error(`Не вдалося завантажити GIF: ${gifUrl}`); loader.style.display = 'none'; };

    if (loadPreferences && currentUserProfileData?.exercise_preferences_summary) {
        const preferences = currentUserProfileData.exercise_preferences_summary.find(p => p.gif_name === gif.name);
        ...
        if (preferences?.reps?.length > 0) {
            ...
            const timeSelects = setsTableContainer.querySelectorAll('.time-select');
            for (let i = 0; i < numSets; i++) {
                if (repsSelects[i] && preferences.reps) repsSelects[i].value = preferences.reps[i] ?? "";
                if (weightsSelects[i] && preferences.weights) weightsSelects[i].value = preferences.weights[i] ?? "";
                if (timeSelects[i] && preferences.time) timeSelects[i].value = preferences.time[i] ?? "";
            }
        } else {
            const defaultSets = 4;
    ...
    if (shouldScroll) {
        setTimeout(() => { fieldset.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 100);
    }
...
function saveUserWorkoutDraft() {
    ...
    userWorkoutDraftTimeout = setTimeout(() => {
        ...
        const draftData = {
            ...
            exercises: Array.from(form.querySelectorAll('#user-exercises-container .exercise')).map(fs => {
                const restMinutes = parseInt(fs.querySelector('.rest-time-minutes')?.value || '0');
                const restSeconds = parseInt(fs.querySelector('.rest-time-seconds')?.value || '0');
                return {
                    ...
                    total_weight: fs.querySelector('.total-weight-input')?.checked ?? false,
                    ...
                    reps: Array.from(fs.querySelectorAll('.reps-select')).map(s => s.value ? parseInt(s.value) : null),
                    weights: Array.from(fs.querySelectorAll('.weight-select')).map(s => s.value ? parseInt(s.value) : null),
                    time: Array.from(fs.querySelectorAll('.time-select')).map(s => s.value ? parseInt(s.value) : null),
                };
            })
        };
        localStorage.setItem(draftKey, JSON.stringify(draftData));
        displayStatus('user-training-plan-message', 'Чернетку збережено.', false, 2000);
    }, 1500);
}
...
function clearUserWorkoutDraft(showMessage = false) {
    ...
    if (showMessage) {
        displayStatus('user-training-plan-message', 'Чернетку видалено.', false, 2000);
    }
}
...
async function handleUserTrainingPlanSubmit(event) {
    ...
    if (!trainingPlanData.title) validationErrors.push("Вкажіть назву тренування.");
    if (!trainingPlanData.date) validationErrors.push("Вкажіть дату тренування.");
    const exerciseFieldsets = document.querySelectorAll('#user-exercises-container .exercise');
    ...
    exerciseFieldsets.forEach((fs, index) => {
        ...
        if (!gifId) validationErrors.push(`Вправа №${exerciseNumber}: не обрано GIF.`);
        if (!name) validationErrors.push(`Вправа №${exerciseNumber}: не вказано назву.`);
        if (sets <= 0) validationErrors.push(`Вправа №${exerciseNumber}: кількість підходів має бути більше 0.`);
        const restMinutes = parseInt(fs.querySelector('.rest-time-minutes')?.value || '0');
        const restSeconds = parseInt(fs.querySelector('.rest-time-seconds')?.value || '0');
    ...
    if (validationErrors.length > 0) {
        displayStatus(messageDivId, "Будь ласка, виправте помилки:\n" + validationErrors.join("\n"), true);
        return;
    }
    try {
        let responseData, response;
        if (currentEditingUserPlanId) {
            const result = await fetchWithAuth(`${baseURL}/training_plans/${currentEditingUserPlanId}`, { method: 'PUT', body: JSON.stringify(trainingPlanData) });
            responseData = result.data; response = result.response;
        } else {
            const result = await fetchWithAuth(`${baseURL}/training_plans`, { method: 'POST', body: JSON.stringify(trainingPlanData) });
            responseData = result.data; response = result.response;
        }
        if (!response.ok) {
            throw new Error(responseData.detail || `Помилка сервера: ${response.status}`);
        }
        const successMessage = currentEditingUserPlanId ? "Тренування успішно оновлено!" : "Якісного тренування, друже!";
...
async function handleEditWorkout(planId) {
    displayStatus('workout-list-status', 'Завантаження тренування для редагування...', false);
    try {
        const { data: planToEdit, response } = await fetchWithAuth(`${baseURL}/training_plans/${planId}`);
        if (!response.ok) { throw new Error(planToEdit.detail || `Помилка завантаження тренування: ${response.status}`); }
        const preparedData = {
            id: planToEdit.id,
        ...
        displayStatus('user-training-plan-message', 'Тренування завантажено. Внесіть зміни та збережіть.', false, 4000);
        const formContainer = document.getElementById('workout-form-container-user');
        if (formContainer) {
...
async function handleDuplicateWorkout(planId) {
    if (!confirm("Створити копію цього тренування?")) { return; }
    displayStatus('workout-list-status', 'Створення дублікату...', false);
    try {
        const { data: planToDuplicate, response } = await fetchWithAuth(`${baseURL}/training_plans/${planId}`);
        if (!response.ok) { throw new Error(planToDuplicate.detail || `Помилка завантаження тренування: ${response.status}`); }
        if (!currentUserProfileData) {
            currentUserProfileData = await fetchCurrentProfileDataOnce();
        }
        const preparedData = {
            ...
            exercises: planToDuplicate.exercises.map(ex => {
                const userPrefs = currentUserProfileData?.exercise_preferences_summary?.find(p => p.gif_name === ex.gif.name);
                const hasValidPrefs = userPrefs && Array.isArray(userPrefs.reps) && userPrefs.reps.length > 0;
                return {
                    gif_id: ex.gif.id,
                    name: ex.gif.name,
                    description: ex.gif.description,
                    ...
                    weights: hasValidPrefs ? userPrefs.weights : Array(ex.sets).fill(null),
        ...
        showUserWorkoutView('form');
        await setupUserWorkoutForm(preparedData);
        displayStatus('user-training-plan-message', 'Дублікат тренування створено. Перевірте та збережіть.', false, 4000);
        const formContainer = document.getElementById('workout-form-container-user');
        if (formContainer) {
            formContainer.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
...
async function populateUserFormWithAIData(aiData) {
    ...
    const form = document.getElementById('independent-workout-form');
    const exercisesContainer = document.getElementById('user-exercises-container');
    if (!form || !exercisesContainer) {
        console.error("[AI User] Не знайдено форму або контейнер вправ для заповнення.");
        return;
    }
    ...
    if (aiData.exercises && aiData.exercises.length > 0) {
        ...
        const sortedExercises = aiData.exercises.sort((a, b) => (a.order || 0) - (b.order || 0));
        ...
        for (const exercise of sortedExercises) {
            ...
            await addUserExerciseToFormWithData({ gif_id: exercise.gif_id, ...exercise }, false);
            ...
            if (exercise.based_on_preference === true) {
                ...
                if (lastExerciseBlock) {
                    const tableContainer = lastExerciseBlock.querySelector('.sets-table-container');
                    if (tableContainer) {
    ...
    saveUserWorkoutDraft();
    alert("Тренування від Gemini успішно згенеровано! Перевірте дані, встановіть бажану дату та збережіть тренування.");
}
...
async function handleUserGeminiGeneration() {
    ...
    if (!promptText) {
        displayStatus(statusDiv.id, 'Будь ласка, введіть опис тренування.', true, 3000);
        return;
    }
    ...
    displayStatus(statusDiv.id, 'Звертаємось до Gemini ✨. Генерація тренування...', false);

    // Таймер, який покаже додаткове повідомлення, якщо процес затягнеться
    const longProcessTimer = setTimeout(() => {
        displayStatus(statusDiv.id, 'Процес триває... Gemini аналізує ваш запит та підбирає найкращі вправи. Це може зайняти до хвилини.', false);
    ...
    try {
        ...
        if (!response.ok) {
            const error = new Error(generatedWorkout.detail || `Помилка сервера: ${response.status}`);
        ...
        displayStatus(statusDiv.id, 'Тренування успішно згенеровано! Перевірте та збережіть.', false, 5000);
        if (inputSection) inputSection.style.display = 'none';

    } catch (error) {
        console.error("Помилка генерації тренування через Gemini для користувача:", error);
...
function generateEditableSetsTableHTML(exercise) {
    ...
    const hasActualDataInPlan = (arr) => Array.isArray(arr) && arr.some(val => val !== null && val !== undefined);
    ...
    for (let i = 0; i < numSets; i++) {
        ...
        if (showRepsMetric) {
            const planRepDisplay = (plannedReps[i] !== null && plannedReps[i] !== undefined) ? plannedReps[i] : '--';
        ...
        if (showWeightsMetric) {
            ...
            const planWeightDisplay = (planWeightVal !== null && planWeightVal !== undefined)
        ...
        if (showTimeMetric) {
            ...
            const planTimeDisplay = (planTimeVal !== null && planTimeVal !== undefined) ? `${planTimeVal} сек` : '--';
...
function addEditListenersToExercise(exerciseDiv) {
    ...
    function setupEditableCell(cellSelector, valueClass, selectClass, preferenceType) {
        ...
        exerciseDiv.querySelectorAll(cellSelector).forEach(cell => {
            ...
            if (!valueSpan || !editSelect) {
                ...
                console.warn(`Елементи для ${cellSelector} не знайдені у вправі ID: ${exerciseDiv.dataset.exerciseId}`);
                return;
            }
            ...
            cell.addEventListener('click', (e) => {
                ...
                if (preferenceType === 'weight') {
                    currentValueForSelect = currentValueForSelect.replace(/\s*кг$/, '').trim();
                } else if (preferenceType === 'time') {
                    currentValueForSelect = currentValueForSelect.replace(/\s*сек$/, '').trim();
                }
                editSelect.value = currentValueForSelect === '--' ? "" : currentValueForSelect;
            ...
            editSelect.addEventListener('change', async () => {
                ...
                const exerciseTable = exerciseDiv.querySelector('.exercise-sets-table.editable');
                // Надійне отримання numSets з data-атрибута таблиці,
                // який встановлюється в generateEditableSetsTableHTML
                const numSets = exerciseTable ? (parseInt(exerciseTable.dataset.numSets) || 0) : 0;

                if (!gifId || numSets === 0) {
                    console.error("addEditListenersToExercise: gifId або numSets не визначені.", { gifId, numSets });
                    alert("Помилка: Неможливо зберегти дані, відсутня інформація про вправу.");
                    return;
                }
                ...
                const repsValueSpans = exerciseDiv.querySelectorAll('.editable-reps .set-reps-value');
                if (repsValueSpans.length > 0) { // Якщо стовпець повторень існує
                ...
                const weightsValueSpans = exerciseDiv.querySelectorAll('.editable-weight .set-weight-value');
                if (weightsValueSpans.length > 0) { // Якщо стовпець ваги існує
                    collectedWeights = Array.from(weightsValueSpans).map((span, index) => {
                        const text = span.textContent.replace(/\s*кг$/, '').trim();
                        return text === '--' ? null : parseInt(text);
                    });
                }

                const timeValueSpans = exerciseDiv.querySelectorAll('.editable-time .set-time-value');
                if (timeValueSpans.length > 0) { // Якщо стовпець часу існує
                ...
                console.log("Дані, зібрані для PUT (з addEditListenersToExercise, після зміни селекта):",
                    { gifId, collectedReps, collectedWeights, collectedTime, numSets });

                // Викликаємо оновлення переваг
                await updateExercisePreference(gifId, collectedReps, collectedWeights, collectedTime, cell);
            });
    ...
    setupEditableCell('.editable-reps', '.set-reps-value', '.reps-edit-select', 'reps');
    setupEditableCell('.editable-weight', '.set-weight-value', '.weight-edit-select', 'weight');
    setupEditableCell('.editable-time', '.set-time-value', '.time-edit-select', 'time');
}
 ...
 */
async function updateExercisePreference(gifId, repsArray, weightsArray, timeArray, feedbackElement = null) {
    //console.log(`Виклик updateExercisePreference для GIF ID: ${gifId}`, { repsArray, weightsArray, timeArray, feedbackElement });

    if (!gifId) { /* ... помилка ... */ return Promise.reject(new Error("Немає ID вправи")); }

    // Перевіряємо, чи всі масиви є масивами і мають однакову довжину (якщо не порожні)
    const isValidArray = (arr) => Array.isArray(arr);
    if (!isValidArray(repsArray) || !isValidArray(weightsArray) || !isValidArray(timeArray) ||
        !(repsArray.length === weightsArray.length && weightsArray.length === timeArray.length) ||
        repsArray.length === 0) { // Має бути хоча б один сет
        console.error("Некоректні дані для оновлення переваг: масиви відсутні, порожні або різної довжини.");
        // alert("Помилка: некоректні дані для збереження.");
        return Promise.reject(new Error("Некоректні дані для оновлення переваг (масиви)"));
    }

    try {
        const { data: updatedPreference, response } = await fetchWithAuth(`${baseURL}/exercises/preferences/${gifId}`, {
            method: "PUT",
            body: JSON.stringify({
                reps: repsArray,
        ...
        if (feedbackElement && feedbackElement instanceof HTMLElement) {
            ...
            console.error("Застосування класу помилки до комірки:", errorClass, feedbackElement);
...
async function handleSavePreferenceClick(event) {
    ...
    if (!gifId || !exerciseDiv || !exerciseId || !planId) {
        console.error("handleSavePreferenceClick: Не вдалося отримати gifId, exerciseDiv, exerciseId або planId.", { gifId, exerciseId, planId });
    ...
    if (exerciseTable && exerciseTable.dataset.numSets) {
        ...
        numSets = plannedReps.length || plannedWeights.length || plannedTime.length || 0;
    }
    if (numSets === 0 && (plannedReps.length > 0 || plannedWeights.length > 0 || plannedTime.length > 0)) {
        numSets = Math.max(plannedReps.length, plannedWeights.length, plannedTime.length);
    }
    ...
    } else if (numSets > 0) { currentReps = Array(numSets).fill(null); }
    ...
    } else if (numSets > 0) { currentWeights = Array(numSets).fill(null); }
    ...
    } else if (numSets > 0) { currentTime = Array(numSets).fill(null); }

    if (numSets > 0 && (currentReps.length !== numSets || currentWeights.length !== numSets || currentTime.length !== numSets)) {
        console.error("Критична помилка збору даних: фінальна довжина масивів не відповідає numSets.", { numSets, currentReps, currentWeights, currentTime });
    ...
    try {
        ...
        if (numSets > 0 || (numSets === 0 && false /* ваша логіка для вправ без підходів */)) {
            await updateExercisePreference(gifId, currentReps, currentWeights, currentTime, null);
        } else if (numSets === 0) {
        ...
        setTimeout(() => {
            ...
            localStorage.setItem(completedExercisesDataKey, JSON.stringify(completedExercisesMap));
            ...
            const setsTableContainer = exerciseDiv.querySelector('.sets-table-container');
            if (setsTableContainer) {
                setsTableContainer.innerHTML = generateUserReadOnlyTableHTML(
                    plannedReps, plannedWeights, plannedTime,
                    currentReps, currentWeights, currentTime
                );
        ...
        console.error("Помилка під час збереження стану вправи (клік на блискавку):", error);
        const errorMsg = (error && error.message) ? error.message : 'Невідома помилка збереження';
        const statusDisplayLocation = exerciseDiv.querySelector('.save-preference-info') || document.getElementById('workout-details-status');

        if (statusDisplayLocation) {
            // ... (ваш код обробки помилки для statusDisplayLocation) ...
            const originalText = statusDisplayLocation.classList.contains('save-preference-info') ? 'Виконав вправу - тисни блискавку:' : '';
            ...
            setTimeout(() => {
                if (originalText && statusDisplayLocation.textContent.startsWith('Помилка:')) {
...
function startSimpleRestTimer(duration) {
    ...
    restTimerInterval = setInterval(() => {
        ...
        timerDisplay.textContent = formatSecondsToMMSS(timeLeftSec > 0 ? timeLeftSec : 0);

        if (timeLeftSec === 10 || timeLeftSec === 3 || timeLeftSec === 2 || timeLeftSec === 1) {
            playTimerSound('tick');
 ...
 */
function startCountdown(duration, onCompleteCallback, finishSoundType = 'finish1') {
    clearInterval(restTimerInterval);
...
function openRestTimerModal() {
    ...
    if (timerSoundToggleBtn) {
        ...
        timerSoundToggleBtn.title = areTimerSoundsEnabled ? "Вимкнути звук" : "Увімкнути звук";
    }
...
function openTab(event, tabName) {
    ...
    if (tabName === "profile") {
        ...
        const defaultSubTabButton = document.querySelector('#profile .sub-tab-link[onclick*="profile-view"]');
        ...
        const defaultSubTabButton = document.querySelector('#ratings .sub-tab-link');
        if (defaultSubTabButton) {
            openRatingsSubTab({ currentTarget: defaultSubTabButton }, 'leaderboard-total');
        }
...
function openProfileSubTab(event, subTabName) {
    ...
    if (subTabName === 'profile-view') {
        ...
        const selectedUserProfileDiv = document.getElementById('community-selected-user-profile');
        ...
        if (token && refreshTokenExists) {
            try {
                ...
                if (!response.ok && response.status !== 401) {
                    console.warn(`Помилка відповіді сервера при виході: ${response.status}`);
                }
            } catch (error) {
...
if (loginForm) {
    loginForm.addEventListener("submit", async (e) => {
        ...
        if (!phone || !password) {
            if (responseDiv) responseDiv.innerText = "Будь ласка, введіть номер телефону та пароль.";
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (responseDiv) responseDiv.innerText = data.detail || "Помилка входу...";
            }
        } catch (error) {
...
async function runInitialChecksAndModals() {
    ...
    const performChecks = async () => {
        const daySelectorOverlay = document.getElementById('day-selector-modal-overlay');
        const newPlanOverlay = document.getElementById('new-plan-notification-overlay');

        if (daySelectorOverlay && newPlanOverlay) {
            try {
                const [{ data: profileData }, { data: plansData }, { response: subResponse }] = await Promise.all([
                ...
                if (profileData && profileData.registration_type === 'self') {
                    ...
                    const newestPlan = plans.sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0];

                    if (newestPlan && new Date(newestPlan.created_at) > thirtyDaysAgo && !localStorage.getItem(`plan_notified_${newestPlan.id}`)) {
                        const oldestPlan = plans.sort((a, b) => new Date(a.created_at) - new Date(b.created_at))[0];
                    ...
                    if (planToSchedule) {
                        const generationTimestamp = localStorage.getItem(`generation_in_progress_${planToSchedule.id}`);
                        const tenMinutes = 10 * 60 * 1000; // 10 хвилин

                        // Якщо мітка існує і вона не старіша за 10 хвилин, ми не показуємо вікно
                        if (generationTimestamp && (Date.now() - generationTimestamp < tenMinutes)) {
                            console.log(`Генерація тренувань для плану ${planToSchedule.id} вже в процесі. Модальне вікно не буде показано.`);
            ...
            if (attempts < maxAttempts) {
                ...
                console.error("Не вдалося знайти HTML-елементи для модальних вікон після кількох спроб.");
            }
        }
    };
...
function startApp() {
    ...
    const authCheckInterval = setInterval(async () => { // Робимо колбек асинхронним
        attempts++;
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
            ...
            if (isAuthorized()) {
                const expiresAt = getAccessTokenExpiresAt();
                const now = new Date().getTime();
                if (expiresAt && (expiresAt - now) < PROACTIVE_REFRESH_LEAD_TIME_MS * 1.5) {
...
(function waitForDOM() {
    ...
    if (document.getElementById('cabinet') && document.getElementById('login-overlay')) {
